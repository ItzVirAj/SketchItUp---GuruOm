...
const PILL: Record<JobCardBucket, { cls: string; label: string }> = {
  ...
  RUNNING: { cls: 'bg-blue-500/10 text-[#5B75F8] dark:text-[#7B92FF] border-blue-500/30', label: 'Running' },
  QC_HOLD: { cls: 'bg-rose-500/10 text-rose-500 dark:text-rose-400 border-rose-500/30', label: 'QC Hold' },
  NOT_STARTED: { cls: 'bg-slate-100 dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10', label: 'Scheduled' }
};
                      ...
                      <thead>
                        <tr
                          className={`border-b font-mono font-bold uppercase tracking-[0.12em] text-[9px] select-none ${
                            isDarkMode
                              ? 'border-white/[0.06] bg-black/40 text-slate-400'
                        ...
                        {rows.map(jc => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="py-3.5 px-4 whitespace-nowrap">
                                <span
                                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[10px] font-mono font-bold uppercase border ${
                                    isDone
                                ...
                                >
                                  ...
                                  {isDone ? (
                                    ...
                                    <span
                                      className={`w-1.5 h-1.5 rounded-full ${
                                        SEGMENTS.find(s => s.id === bucket)?.dot || 'bg-slate-400'
                                      }`}
                                    />
                                  )}
