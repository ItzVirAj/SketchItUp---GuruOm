...
export async function createProfile(user: Partial<SystemUser> & { password?: string; requirePasswordChangeFirstLogin?: boolean }): Promise<SystemUser> {
  try {
    const res = await apiClient.post<{ user: any }>('/auth/users', {
      name: user.name,
      email: user.email,
      role: user.role,
...
export async function insertVendorBill(bill: VendorBill): Promise<void> {
  try {
    await apiClient.post('/vendor-bills', {
      ...
      grossAmount: bill.grossAmount ?? bill.amount,
      isPurchaseOfGoods: bill.isPurchaseOfGoods ?? false,
      attachmentUrl: bill.attachmentId
    });
  } catch (err) {
...
export interface ScannedReceiptFields {
...
/**
 * Uploads a photo/PDF of a vendor bill or receipt and returns AI-extracted fields
 * to pre-fill the "Record Vendor Bill" form, plus a reference to the stored attachment.
 * The extraction is always a draft for the user to review — nothing is saved until
 * the bill is submitted through the normal form flow.
 */
export async function scanVendorBillReceipt(
  file: File,
  draftBillNo: string
): Promise<{ extracted: ScannedReceiptFields; attachmentId: string | null }> {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('draftBillNo', draftBillNo);

  const response = await apiClient.postFormData<{ data: { extracted: ScannedReceiptFields; attachmentId: string | null } }>(
    '/vendor-bills/scan-receipt',
    formData
  );
...
export async function completePdiInspectionForOrder(
  ...
  // C-02: PDI certificate numbers are minted server-side by the atomic document
  // sequence — the client never generates a certificate number. If no number is
  // supplied, return '' and let the backend's assigned value surface on reload.
  const certNo = payload.certificateNo || '';
...
export async function generateInvoiceForOrder(
  ...
  // C-01/C-02: the GST invoice number is assigned by the backend's atomic sequence.
  // The client must never mint fallback numbers; the server's returned invoiceNo
  // (captured below) is what the UI will show after reload.
  const invoiceNo = invoiceData.invoiceNo || 'PENDING-SERVER-ASSIGNMENT';
...
export async function generateChallanForOrder(
  ...
  // C-02: challan numbers are minted server-side by the atomic document sequence.
  const challanNo = challanData.challanNo || 'PENDING-SERVER-ASSIGNMENT';
