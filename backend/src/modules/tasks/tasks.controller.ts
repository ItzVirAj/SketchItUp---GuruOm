...
function actorFromReq(req: Request) {
  ...
  // req.user.id is the canonical authenticated-user ID
  // populated by auth.middleware.ts from the JWT `sub` claim.
  if (!user?.id) {
...
export class TasksController {
  ...
  async addComment(req: Request, res: Response) {
    try {
      ...
      const data = await tasksService.addComment(
        req.params.id,
        body,
        actor
      );
  ...
  async cancelTask(req: Request, res: Response) {
    try {
      ...
      const data = await tasksService.cancelTask(
        req.params.id,
        actor,
        req.body?.reason
      );
